error id: file:///D:/Doneload/ITSD-DT2025-26-Template/app/events/EndTurnClicked.java:_empty_/GameState#aiPlayer#getHand#
file:///D:/Doneload/ITSD-DT2025-26-Template/app/events/EndTurnClicked.java
empty definition using pc, found symbol in pc: _empty_/GameState#aiPlayer#getHand#
    ...
    private void executeAITurn(ActorRef out, GameState gameState) {
        ...
        for (int k = 0; k < 1; k++) {
            if (!gameState.aiPlayer.getDeck().isEmpty()) {
                if (gameState.aiPlayer.getHa@@nd().size() < 6) {
    ...
    private boolean castSmartAIAbility(ActorRef out, GameState gameState, Card card) {
        ...
        if (name.equals("True Strike") || name.equals("Truestrike")) {
            ...
            for (GameUnit e : getAllEnemyUnits(gameState)) {
                // 1. 优先斩杀：血量 <= 2 的直接带走
                if (e.getCurrentHealth() <= 2) { 
                    target = e; 
                    break; // 找到斩杀目标直接退出循环
                }
                // 2. 其次选择：如果没有斩杀目标，找攻击力最高的单位削血
                if (e.getCurrentAttack() > maxAttack) {
                    maxAttack = e.getCurrentAttack();
                    target = e;
                }
            }
            
            // 3. 保底策略：如果场上没怪（或者没选到），直接打英雄脸
