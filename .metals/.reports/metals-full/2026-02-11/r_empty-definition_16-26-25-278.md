error id: file:///D:/Doneload/ITSD-DT2025-26-Template/app/structures/AttackLogic.java:_empty_/GameUnit#getCurrentHealth#
file:///D:/Doneload/ITSD-DT2025-26-Template/app/structures/AttackLogic.java
empty definition using pc, found symbol in pc: _empty_/GameUnit#getCurrentHealth#
...
// [文件: structures/AttackLogic.java]
...
public static void executeAttack(GameUnit attacker, GameUnit defender, ActorRef out, GameState gameState) {
    ...
    if (MovementRule.isProvoked(gameState.board, attacker)) {
        if (!MovementRule.isProvokeUnit(defender)) {
            ...
            if (attacker.getOwner() == gameState.humanPlayer) {
                ...
                BasicCommands.addPlayer2Notification(out, "Must attack Provoker!", 2);
            }
            return; 
        }
    }
    ...
    if (attacker.getOwner() == gameState.humanPlayer) {
        ...
        BasicCommands.addPlayer2Notification(out, "Attacking!", 2);
    }
    // ----------------------------------------------
    ...
    if (defender.getCurrentHealth() > 0 && attacker.getCurrentHealt@@h() > 0) {
    ...
    // (后续如果还有 Horn of the Forsaken 的逻辑，也请记得把 addPlayer1Notification 改成判断归属的写法)
}
