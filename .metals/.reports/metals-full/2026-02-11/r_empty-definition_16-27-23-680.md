error id: file:///D:/Doneload/ITSD-DT2025-26-Template/app/events/EndTurnClicked.java:_empty_/GameState#board#getYMax#
file:///D:/Doneload/ITSD-DT2025-26-Template/app/events/EndTurnClicked.java
empty definition using pc, found symbol in pc: _empty_/GameState#board#getYMax#
...
// [文件: events/EndTurnClicked.java]
    ...
    for (GameUnit enemy : targets) {
        ...
        if (canKill) {
            score += 150.0;
            if (isEnemyHero) score += 100000.0; // 斩杀英雄 = 胜利
        }

        // B. 生存判定 (Survival)
        // 规则：如果没有击杀敌人，且敌人不是晕眩状态，敌人会反击
        ...
        if (willDie) {
            ...
            if (amIHero) {
                ...
                // 普通单位，严重扣分，除非是换掉高价值单位
                score -= 100.0; 
                // 但如果我只剩1血，反正也是死，不如换点血，扣分少点
                if (myHp == 1) score += 50.0; 
            }
        } else if (!willBeCountered) {
            // "白嫖"：我打他，他不会反击（死了或者晕了），加分！
            score += 40.0;
        }
        ...
        // D. 伤害溢出惩罚 (Overkill)
        // 如果我 10 攻，你 1 血，有点浪费。稍微扣一点分，让别的单位来收割
        if (canKill && (myAtk - enemyHp > 3) && !isEnemyHero) {
            score -= 10.0; 
        }

        // E. 英雄修正
        if (isEnemyHero) score += 35.0; // 总是倾向于打脸

        // F. 距离修正
        score -= (distance * 3.0);
        if (distance <= 1) score += 15.0; // 贴脸优先
    ...
    // 如果最高分还是负得离谱（比如全是送死局），那就忍住不打，等待机会
...
private Tile findValidSpawnTile(GameState gameState) {
    ...
    // 初始化比较值
    double bestDistanceMetric = defensiveMode ? 9999.0 : 9999.0;
    ...
    for (int x = 0; x < gameState.board.getXMax(); x++) {
        for (int y = 0; y < gameState.board.getYMax(); y++) {
            ...
            if (defensiveMode) {
                // 【防御模式】：找离我最近，且稍微靠向敌人的位置（护盾）
                // 评分：离我越近越好(权重高)，离敌人越近越好(权重低，用于挡在中间)
...
private int getUnitY(GameState gameState, GameUnit unit) {
    for (int x = 0; x < gameState.board.getXMax(); x++) {
        for (int y = 0; y < gameState.board.g@@etYMax(); y++) {
            // 查棋盘最准！
